...
public interface EstimateRepository extends JpaRepository<EstimateEntity, Integer> {
        ...
        // 견종별 모든 견적 정보 출력
        @Query(nativeQuery = true, value = "select e.name, e.category_first, e.category_second, round( ((d.weight_min+d.weight_max) /2)*0.025*30 * (e.price_avg/e.weight),0) as dogFeedPrice, if(e.small is null, price_avg, Case when(d.health_Size = 1) then e.small  when(d.health_Size between 2 and 3) then e.medium  when(d.health_Size between 4 and 5) then e.large  else 0 end ) as productPrice,  round((price_avg/weight),0) as ProductPricePerWeight , e.image, e.comment from estimate e join dog d where d.name = :name order by category_first, category_second")
