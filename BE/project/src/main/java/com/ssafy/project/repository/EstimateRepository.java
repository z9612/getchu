...
public interface EstimateRepository extends JpaRepository<EstimateEntity, Integer> {
    ...
    public List<PriceResult> getFeedPrice(String name);
     ...
     //의료 정보 출력
     @Query(nativeQuery = true, value = "select name, format(e.price_min,0) as min, format(e.price_avg,0) as avg ,format(e.price_max,0) as max from estimate e where category_second = 'vaccination'")
